...
export default function Login() {
  ...
  return (
    ...
    >
      <form
        onSubmit={submit}
        className="w-full max-w-[440px] rounded-[20px] p-8"
        style={{
      ...
      >
        <h1 className="mb-6 text-[28px] font-extrabold text-white">Login</h1>

        <label className="mb-2 block text-[15px] font-bold text-gold">ID Pengguna</label>
        <input
          value={id}
          onChange={(e) => setId(e.target.value)}
          autoFocus
          autoComplete="username"
          className="mb-5 h-14 w-full rounded-xl px-4 text-[15px] text-white outline-none focus:ring-2 focus:ring-gold/40"
          style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(201,169,59,0.55)' }}
        />

        <label className="mb-2 block text-[15px] font-bold text-gold">Password</label>
        <div className="relative mb-2">
          <input
            ...
            className="h-14 w-full rounded-xl px-4 pr-12 text-[15px] text-white outline-none focus:ring-2 focus:ring-gold/40"
            style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(201,169,59,0.55)' }}
          />
          <button
            type="button"
        ...
        <button
          type="submit"
          disabled={busy}
          className="mt-6 h-14 w-full rounded-xl text-[16px] font-extrabold text-[#1A2A1F] transition-colors disabled:opacity-70"
